...
import org.jext.*;
//import org.jext.gui.*;
import de.schlund.pfixeditor.view.JextMenu;
...
public class GUIUtilities
{
  ...
  {
    if (!Props.getBooleanProperty("scrollableTabbedPanes"))
      return;
  ...
  public static void saveGeometry(Window win, String name)
  {
    ...
    Props.setProperty(name + ".width", String.valueOf(size.width));
    Props.setProperty(name + ".height", String.valueOf(size.height));
    ...
    if (y < -4)
      y = -4;
    Props.setProperty(name + ".x", String.valueOf(x));
    Props.setProperty(name + ".y", String.valueOf(y));
  }
  ...
  public static void loadGeometry(Window win, String name)
  {
    ...
    try
    {
      width = Integer.parseInt(Props.getProperty(name + ".width"));
      height = Integer.parseInt(Props.getProperty(name + ".height"));
    ...
    try
    {
      x = Integer.parseInt(Props.getProperty(name + ".x"));
      y = Integer.parseInt(Props.getProperty(name + ".y"));
    } catch (NumberFormatException nf) {
  ...
  public static JMenu loadMenu(String name, boolean isLabel)
  {
    ...
    if (!isLabel)
    {
      label = Props.getProperty(name + ".label");
      if (label == null)
        label = name;
    }
    else
  ...
  {
    String name = Props.getProperty(action.concat(".label"));
    if (name == null)
...
/*
    if (picture != null)
    {
      ...
      ImageIcon icon = Utilities.getIcon(picture.concat(Props.getProperty("jext.look.icons")).concat(".gif"),
                       Jext.class);
      if (icon != null)
       mi.setIcon(icon);
...
*/
    ...
    if (action != null)
    {
      MenuAction a = Gui.getAction(action);
      if (a == null)
        mi.setEnabled(false);
      else
      {
