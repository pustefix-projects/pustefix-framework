...
public class URLRewriteSessionStrategy implements SessionTrackingStrategy {
    ...
    public URLRewriteSessionStrategy(SessionStrategyContext context) {
    ...
    public void handleRequestByStrategy(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        if (req.isRequestedSessionIdValid()) {
            ...
            Boolean secure = (Boolean) session.getAttribute(context.getSessionAdmin().SESSION_IS_SECURE);
            ...
            if (!runningUnderSSL(req) && context.getSessionAdmin().idWasParentSession(req.getRequestedSessionId())) {
        ...
        if (!has_session && !context.needsSession() && context.needsSSL(preq) && !runningUnderSSL(req)) {
            LOG.debug("=> VI");
    ...
    private void redirectToInsecureSSLSession(PfixServletRequest preq, HttpServletRequest req, HttpServletResponse res, String msanc) {
        ...
        session.setAttribute(context.getSessionAdmin().SESSION_IS_SECURE, Boolean.FALSE);
    ...
    private void redirectToSecureSSLSession(PfixServletRequest preq, HttpServletRequest req, HttpServletResponse res, String msanc) {
        ...
        session.setAttribute(context.getSessionAdmin().PARENT_SESS_ID, old_id);
        if (visit_id != null) {
        ...
        session.setAttribute(context.getSessionAdmin().SESSION_IS_SECURE, Boolean.TRUE);
        session.setAttribute(STORED_REQUEST, preq);
