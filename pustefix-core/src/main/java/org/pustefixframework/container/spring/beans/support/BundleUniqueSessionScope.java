...
/**
 * Scope overriding the default session scope and making the bean names unique per bundle
 * to avoid that beans having the same name are overwritten by beans from different bundles
 * when they're stored as attribute in the current session.
 * 
 * @author mleidig@schlund.de
 *
 */
public class BundleUniqueSessionScope extends SessionScope {

    private long bundleId;
    
    public BundleUniqueSessionScope(boolean global, long bundleId) {
        super(global);
