...
<xsl:if test="$currentprj/p:application/p:default-path/node()">
  ...
  </xsl:choose>://%{SERVER_NAME}<xsl:if test="not(normalize-space(substring-after($address,':'))='80' or normalize-space(substring-after($address,':'))='443')">:%{SERVER_PORT}</xsl:if><xsl:apply-templates select="$currentprj/p:application/p:default-path/node()"/> [NC,R=301,L]
</xsl:if>
