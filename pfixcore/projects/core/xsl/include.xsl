...
<xsl:stylesheet version="1.1"
  ...
  <xsl:param name="__editor_include_parts_editable_by_default" select="string(not(document('servletconf/projects.xml')/projects/common/editor/include-parts-editable-by-default/text() = 'false'))"/>
  ...
  <xsl:template name="pfx:missinc">
    ...
    <xsl:message>*** Include not found:
      TargetGenerator: <xsl:value-of select="$__target_gen"/>
        ...
        <xsl:variable name="path">
          <xsl:choose>
            <xsl:when test="starts-with($src, '/')">
              <xsl:value-of select="substring-after($src, '/') and not(starts-with($src, '//'))"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$src"/>
            </xsl:otherwise>
