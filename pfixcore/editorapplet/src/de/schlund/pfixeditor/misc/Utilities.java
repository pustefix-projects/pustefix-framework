...
public class Utilities
{
  ...
  {
    JOptionPane.showMessageDialog(null, message, "utils.message",
  ...
  {
    JOptionPane.showMessageDialog(null, message, "utils.error",
  ...
  public static void setCursorOnWait(Component comp, boolean on)
  {
    if (on)
    {
      if (comp instanceof Gui)
        ((Gui) comp).showWaitCursor();
      else
        comp.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
    } else {
      if (comp instanceof Gui)
        ((Gui) comp).hideWaitCursor();
      else
