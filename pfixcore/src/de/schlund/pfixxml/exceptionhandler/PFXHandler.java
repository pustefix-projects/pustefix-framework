...
class PFXHandler {
    ...
    protected void xhandle(ExceptionContext excontext) {
        ...
        int result =0;
        Exception e=excontext.getException();
        
        if(errorflag_==true) {
            pfutil_.error(
                    "exceptionhandler not configured! Logging to file only.");
        ...
        if(propman_.isInitialised()&&gconf_.isUseme()) {
            for(Enumeration enum=instancecontainerlist_.elements();
                enum.hasMoreElements();) {
                InstanceCheckerContainer checker=(InstanceCheckerContainer) enum.nextElement();
                result=checker.doesMatch(e);
                pfutil_.debug(
                        "The InstanceCheckerContainers returned: " + 
                        pfutil_.strByResult(result));
    ...
    void init() {
        ...
        if(initialised_) {
            ...
            ReportGeneratorTask rg=new ReportGeneratorTask(
                                           instancecontainerlist_, info);
        ...
        for(int i=0; i<econf.length; i++) {
            ...
            ic=new InstanceCheckerContainer(econf[i].getType(), 
                                            econf[i].getMatch(), 
                                            econf[i].getLimit(), 
    ...
    private void internalError(String message) {
        ...
        buf.append("Useme flag=" + new Boolean(gconf_.isUseme()).toString() + 
                   "\n");
        pfutil_.error(
                "Internal Error during ExceptionHandling!!!\n" + 
                buf.toString());
    }
    ...
    private void ruleTriggerMatch(ExceptionContext excontext) {
        ...
        ReportGeneratorTask rgtask=null;
        pfutil_.debug(
                " ReportGeneratorTask triggert from " + e.toString() + 
                " at " + excontext.getDate());
