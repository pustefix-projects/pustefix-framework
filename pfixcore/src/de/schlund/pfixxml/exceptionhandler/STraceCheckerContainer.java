...
/**
 * @author <a href="mailto: haecker@schlund.de">Joerg Haecker </a> <br/>This
 *             class handles incoming exceptions which will be checked by their
 *             stacktrace.
 */
class STraceCheckerContainer {
    ...
    private StackTraceElement[] strace_ = null;
     ...
     * @param limit
     *                 see
     *                 <see>InstanceCheckerContainer#InstanceCheckerContainer(...)
     *                 </see>
     * @param limit_dim
     *                 see
     *                 <see>InstanceCheckerContainer#InstanceCheckerContainer(...)
     ...
     */
    void setStrace(StackTraceElement[] strace) {
        this.strace_ = strace;
    }
    /**
     ...
     * 
     * @return An int as result. Returns FULL if the limit is exeeded. Returns
     *             MATCH if limit is not exeeded. Returns TRIGGER_FULL if limit is
     *             not exceeded but a generation of a report should be triggered.
     */
    int doesMatch() {
        PFUtil.getInstance().debug("StraceChecker at entry : currentburst: " + currentburst_);
        long nowmilli = 0;
        int diff = 0;
        int rate = 0;
        float sdiff = 0;
        ...
        PFUtil.getInstance().debug("StraceChecker at return : currentburst: " + currentburst_);
        return status;
    }
    /**
