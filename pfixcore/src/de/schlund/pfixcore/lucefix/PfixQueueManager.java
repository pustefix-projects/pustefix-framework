...
public class PfixQueueManager implements Runnable {
     ...
     * @throws XMLException
     */
    public PfixQueueManager(Integer idletime) throws XMLException {
    ...
    public void run() {
        ...
        while (true) {
            ...
            while ((current = queue.next()) != null) {
                try {
                    if (current.getType() == Tripel.Type.INSERT || current.getType() == Tripel.Type.EDITORUPDATE) {
                        try {
                        ...
                        if (hits.length() == 0) {
                            ...
                            if (f.lastModified() == DateField.stringToTime(hits.doc(0).get("lasttouch")))
                                ...
                                // ts differs, remove outdaten from index and
                                // add the new
                                Document newDoc = cache.getDocument(current);
                                documents2write.add(newDoc);
                                cache.remove(newDoc);
            ...
            try {
                ...
                if (documents2write.size() > 0) {
                    if (writer == null) writer = new IndexWriter(lucene_data_path, analyzer, false);
     ...
     * @throws XMLException
     */
    public static synchronized PfixQueueManager getInstance(Integer idletime) throws XMLException {
        if (_instance == null) _instance = new PfixQueueManager(idletime);
        return _instance;
    }
