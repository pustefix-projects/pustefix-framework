...
public class IWrapperFileScanner {
    ...
    public List<File> getChangedFiles(File srcDir,File destDir) {
        List<File> changedFiles=new ArrayList<File>();
        getChangedFiles(srcDir,destDir,changedFiles);
        return changedFiles;
    }
    
    private void getChangedFiles(File srcDir,File destDir,List<File> changedFiles) {
        File[] srcFiles=srcDir.listFiles();
        for(File srcFile:srcFiles) {
            if(srcFile.isDirectory()) {
                if(!(srcFile.getName().equals(".svn")||srcFile.getName().equals("CVS"))) {
                    File destFile=new File(destDir,srcFile.getName());
                    getChangedFiles(srcFile,destFile,changedFiles);
                }
            } else {
                        ...
                        try {
                            ...
                            while((line=reader.readLine())!=null) {
                                if(line.contains("IWrapper")) {
                                    if(line.contains("@IWrapper")||
                                        line.contains("@de.schlund.pfixcore.generator.annotation.IWrapper")) {
                                            found=true;
                                            break;
                                        }
                                }
