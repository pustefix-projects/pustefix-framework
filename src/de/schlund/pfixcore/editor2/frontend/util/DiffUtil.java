 ...
 * 
 * 
 * The Java version of the Diff algorithm in this file is taken from
...
public class DiffUtil {
    ...
    /*
     * The code beyond this line is the Diff algorithm from BMSI
     */
    private static class Diff {

        /**
         * Prepare to find differences between two arrays. Each element of the
         * arrays is translated to an "equivalence number" based on the result
         * of <code>equals</code>. The original Object arrays are no longer
         * needed for computing the differences. They will be needed again later
         * to print the results of the comparison as an edit script, if desired.
         */
        ...
        private int[] fdiag; /*
                                 ...
                                 * coordinate of the point furthest along the
                                 * given diagonal in the forward search of the
                                 * edit matrix.
                                 */
        ...
        private int[] bdiag; /*
                                 * Vector, indexed by diagonal, containing the X
                                 * coordinate of the point furthest along the
                                 * given diagonal in the backward search of the
                                 * edit matrix.
                                 */
         ...
         * ends, doing a breadth-first search through the space of
         * edit-sequence. When the two searches meet, we have found the midpoint
         * of the shortest edit sequence.
         * 
         * The value returned is the number of the diagonal on which the
         * midpoint lies. The diagonal number equals the number of inserted
         * lines minus the number of deleted lines (counting only lines before
         * the midpoint). The edit cost is stored into COST; this is the total
         * number of lines inserted or deleted (counting only lines before the
         * midpoint).
         * 
         * This function assumes that the first lines of the specified portions
         * of the two files do not match, and likewise that the last lines do
         * not match. The caller must trim matching lines from the beginning and
         * end of the portions it is going to specify.
         * 
        ...
        private int diag(int xoff, int xlim, int yoff, int ylim) {
            ...
            final int fmid = xoff - yoff; // Center diagonal of top-down
            // search.
            final int bmid = xlim - ylim; // Center diagonal of bottom-up
            // search.
        ...
        private void compareseq(int xoff, int xlim, int yoff, int ylim) {
            ...
            if (xoff == xlim)
                ...
                if (c == 1) {
                    ...
                    /*
                     * Use that point to split this problem into two
                     * subproblems.
                     */
                    compareseq(xoff, b, yoff, b - d);
                    /*
                     * This used to use f instead of b, but that is incorrect!
                     * It is not necessarily the case that diagonal d has a
                     * snake from b to f.
                     */
            ...
            public change build_script(final boolean[] changed0, int len0,
                ...
                while (i0 < len0 || i1 < len1) {
                    if (changed0[1 + i0] || changed1[1 + i1]) {
                        ...
                        /* Record this change. */
                        script = new change(line0, line1, i0 - line0, i1
                                - line1, script);
                    }
            ...
            public change build_script(final boolean[] changed0, int len0,
                ...
                while (i0 >= 0 || i1 >= 0) {
                    if (changed0[i0] || changed1[i1]) {
                        ...
                        /* Record this change. */
                        script = new change(i0, i1, line0 - i0, line1 - i1,
                                script);
                    }
        ...
        /**
         * Get the results of comparison as an edit script. The script is
         * described by a list of changes. The standard ScriptBuilder
        ...
        public change diff(final ScriptBuilder bld) {
            ...
            /*
             * Some lines are obviously insertions or deletions because they
             * don't match anything. Detect them now, and avoid even thinking
             * about them in the main comparison algorithm.
             */
            ...
            /*
             * Modify the results slightly to make them prettier in cases where
             * that can validly be done.
             */
         ...
         * objects. Each change represents one place where some lines are
         * deleted and some are inserted.
         * 
         * LINE0 and LINE1 are the first affected lines in the two files (origin
         * 0). DELETED is the number of lines deleted here from file 0. INSERTED
        ...
        public static class change {
             ...
             * 
             * If DELETED is 0 then LINE0 is the number of the line before which
        ...
        class file_data {
            ...
            void clear() {
                /*
                 * Allocate a flag for each line of each file, saying whether
                 * that line is an insertion or deletion. Allocate an extra
                 * element, always zero, at each end of each vector.
                 */
                changed_flag = new boolean[buffered_lines + 2];
            }

            /**
             * Return equiv_count[I] as the number of lines in this file that
             * fall in equivalence class I.
             * 
             ...
             * insertion so that it will be printed in the output.
             * 
             * @param f
             *            the other file
             */
            ...
            /**
             * Mark to be discarded each line that matches no line of another
             * file. If a line matches many lines, mark it as provisionally
             * discardable.
             * 
             * @see equivCount()
             * @param counts
             *            The count of each equivalence number for the other
             *            file.
            ...
            private void filterDiscards(final byte[] discards) {
                ...
                for (int i = 0; i < end; i++) {
                    ...
                    if (discards[i] == 2)
                        ...
                        /*
                         * Find end of this run of discardable lines. Count how
                         * many are provisionally discardable.
                         */
                        for (j = i; j < end; j++) {
                            if (discards[j] == 0)
                                break;
                        ...
                        if (provisional * 4 > length) {
                            ...
                            for (j = 0, consec = 0; j < length; j++)
                                if (discards[i + j] != 2)
                                    ...
                                    /*
                                     * Back up to start of subrun, to cancel it
                                     * all.
                                     */
                                    j -= consec;
                                else if (minimum < consec)
                             ...
                             * more nonprovisionals in a row or until the first
                             * nonprovisional at least 8 lines in. Until that
             ...
             * possible.
             * 
             * We do something when a run of changed lines include a blank line
             * at one end and have an excluded blank line at the other. We are
             ...
             * exception is if the preceding blank line would join this change
             * to other changes.
             * 
             * @param f
                ...
                for (;;) {
                    ...
                    while (i < i_end && !changed[1 + i]) {
                        while (other_changed[1 + j++])
                            /*
                             * Non-corresponding lines in the other file will
                             * count as the preceding batch of changes.
                             */
                            other_preceding = j;
                        i++;
                    }
                    ...
                    for (;;) {
                         ...
                         * unchanged one, and this run does not follow right
                         * after a previous run, and there are no lines deleted
                         * from the other file here, then classify the first
                         * changed line as unchanged and the following line as
                         * changed in its place.
                         */

                        /*
                         * You might ask, how could this run follow right after
                         * another? Only because the previous run was shifted
                         * here.
                         */
                        ...
                        if (end != i_end
                            ...
                            ++i;
                            /*
                             * Since one line-that-matches is now before this
                             * run instead of after, we must advance in the
                             * other file to keep in synch.
                             */
                            ++j;
                        } else
                            break;
             ...
             * for each line. It is this vector that is actually compared with
             ...
             * lines) to real ones (counting those lines). Both are origin-0.
             */
            ...
            /**
             * Array, indexed by real origin-1 line number, containing true for
